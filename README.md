...
- どのような品質保証をするか
- 具体的にどのようにテストを設計・実装・運用していくか

これらを現場ごとに探る・合意形成を行う必要性がある、というのが結論大方針になるかと思います。
(※後は当たり前ですが、Salesforce専用のテストベンダーに相談するのが最も確実性と専門性の高いソリューションに繋がりそうです。)
...
現状の運用を推察すると、以下2観点のテスト観点を基本に、どこで何をテストするべきかのテスト設計を行うのが良いかと思います。
